...
import com.codingelab.validation.errors.NumbersInRange;
 ...
 *
 */
public final class NumbersOutRange extends _RegexDecorator {
	private final String range;
	private Error errorRange;
	private final String numbersInRange;
	public NumbersOutRange(_Regex compoment,String range,int required)
	...
	//	if(range.equals(null))throw new NullPointerException("Range cannot be empty");
		...
		if(super.findErrorWithException(range,"[\\D]")){
			...
			if(!super.matches(range,"([\\d]([-]?[\\d]{1})?)+")){
				throw new InvalidRange("NumbersOutRange can only contains numbers or valid range of numbers.");
			}
		...
		this.range=range;
		this.numbersInRange="[\\d&&["+this.range+"]]";
		// to Opposite Range exception... 
		super.findErrorWithException("",this.numbersInRange);
		this.errorRange=new NumbersInRange(range);
	}

	// additional error detection
	@Override protected boolean isInvalid(String input){
		return super.findError(input,this.numbersInRange);
	}
	@Override public String repair(String input) {
		try{
			input=super.repairInput(input, this.numbersInRange);
	...
	@Override protected List<Error> getRangeError(String input){
		...
		int times=super.count(input, "[\\d&&["+this.range+"]]");
		Error error=new Error();
		if(times>0){
			error.setTimes(times);
			error.setErrorType(ErrorType.NUMBERS_IN_RANGE);
	...
	@Override protected String typeToString() {
		return RegexType.NUMBERS_OUT_RANGE.toString().toLowerCase()
